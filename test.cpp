...
int create_server_socket(void)
{
    ...
    sa.sin_addr.s_addr = htonl(INADDR_ANY); // 0.0.0.0, localhost
...
void accept_new_connection(int server_socket, fd_set *all_sockets, int *fd_max)
{
	...
	getsockname(client_fd, (struct sockaddr *) &_clientAddr, &len);

	uint32_t addr = ntohl(_clientAddr.sin_addr.s_addr);

	std::cout << "_clientAddr.sin_addr.s_addr = " << _clientAddr.sin_addr.s_addr << std::endl;
	...
	dprintf(2, "clientip = %s\n", clientip.data());
	dprintf(2, "clientport = %d\n", clientport);
